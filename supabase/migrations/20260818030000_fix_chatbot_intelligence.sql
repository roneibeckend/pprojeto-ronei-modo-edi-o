-- Atualizar a resposta de Certificados para ser mais direta e incluir o passo a passo solicitado
...
-- Atualizar a resposta de Planilhas e Materiais para ser mais específica
UPDATE public.knowledge_base
SET 
...
-- Refinar palavras-chave do PWA para evitar falso positivo com "aplicativo" quando o usuário quer algo interno
UPDATE public.knowledge_base
SET 
  keywords = array_remove(keywords, 'baixar'), -- "baixar" é muito comum para PDF/Ebooks
  questions = array_cat(questions, ARRAY['instalar no celular', 'como ter o app'])
WHERE title = 'Como instalar o App (PWA)';

-- Garantir que não haja itens duplicados nos arrays após o array_cat (opcional mas bom para limpeza)
UPDATE public.knowledge_base 
SET 
