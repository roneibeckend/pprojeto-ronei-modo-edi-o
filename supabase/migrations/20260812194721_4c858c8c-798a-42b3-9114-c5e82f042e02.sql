-- Adiciona checkpoints ao ebooks e cursos
ALTER TABLE public.ebooks ADD COLUMN IF NOT EXISTS checkpoints JSONB DEFAULT '[]';
ALTER TABLE public.courses ADD COLUMN IF NOT EXISTS checkpoints JSONB DEFAULT '[]';

-- Garante que a tabela progress_tracking suporte marcos de porcentagem
ALTER TABLE public.progress_tracking ADD COLUMN IF NOT EXISTS last_milestone INTEGER DEFAULT 0;

-- Função para verificar marcos de progresso e criar notificações
...
BEGIN
    ...
    v_last_milestone := COALESCE(NEW.last_milestone, 0);
...
-- Trigger para monitorar marcos
DROP TRIGGER IF EXISTS tr_check_progress_milestones ON public.progress_tracking;
CREATE TRIGGER tr_check_progress_milestones
BEFORE UPDATE ON public.progress_tracking
FOR EACH ROW
WHEN (NEW.item_type IN ('course', 'ebook'))
