-- Drop the view if it somehow exists partially
...
DO $$ BEGIN
    CREATE POLICY "Users can view all stats" ON public.user_stats FOR SELECT TO authenticated USING (true);
...
DROP TRIGGER IF EXISTS on_item_completion ON public.progress_tracking;
...
-- View for Global Ranking (using profiles.name)
