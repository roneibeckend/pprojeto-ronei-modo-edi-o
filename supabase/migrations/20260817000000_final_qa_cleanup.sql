...
-- 5. Note: auth.users deletion usually requires service_role or admin client.
-- In Supabase migrations, we can delete from auth.users.
...
WHERE user_id IN (SELECT id FROM auth.users WHERE email ILIKE '%qa%' OR email ILIKE '%test%')
OR customer_email ILIKE '%qa%' OR customer_email ILIKE '%test%';
