...
function ProgressPage() {
  const started = courses.filter((c: any) => c.progress > 0).length;
  const finished = courses.filter((c: any) => c.progress === 100).length;
  const read = ebooks.filter((e: any) => e.progress === 100).length;
  const unlocked = certificates.filter((c: any) => c.unlocked).length;
