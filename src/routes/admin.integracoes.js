...
import { Sparkles, CreditCard, Settings2, Activity, Loader2, ChevronRight, ShieldCheck, BrainCircuit, Wallet, Save, Globe, Key, Copy, Check, ExternalLink, BookOpen, Terminal, Clock, RotateCcw, Zap, Info, Mail, SendHorizontal, Percent } from "lucide-react";
...
function IntegrationsPage() {
    ...
    const filtered = integrations?.filter(i => {
        if (activeCategory === 'email')
            return false; // email category is handled by panel
        return i.type === activeCategory;
    }) || [];
    return (<div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between border-b border-white/5 pb-8">
        ...
        <div className="flex items-center gap-1 rounded-sm border border-white/5 bg-black/40 p-1 self-start sm:self-auto">
          <Button variant="ghost" onClick={() => { setActiveCategory('ia'); setSelectedItem(null); }} className={`flex items-center gap-2 px-4 py-2 text-[10px] font-bold uppercase tracking-widest transition h-10 ${activeCategory === 'ia' ? 'bg-[#ff6a00] text-black hover:bg-[#ff6a00]' : 'text-white/40 hover:text-white hover:bg-white/5'}`}>
            <BrainCircuit className="h-3.5 w-3.5"/> IA
          </Button>
          <Button variant="ghost" onClick={() => { setActiveCategory('payment'); setSelectedItem(null); }} className={`flex items-center gap-2 px-4 py-2 text-[10px] font-bold uppercase tracking-widest transition h-10 ${activeCategory === 'payment' ? 'bg-[#ff6a00] text-black hover:bg-[#ff6a00]' : 'text-white/40 hover:text-white hover:bg-white/5'}`}>
            <Wallet className="h-3.5 w-3.5"/> Pagamentos
          </Button>
          <Button variant="ghost" onClick={() => { setActiveCategory('email'); setSelectedItem(null); }} className={`flex items-center gap-2 px-4 py-2 text-[10px] font-bold uppercase tracking-widest transition h-10 ${activeCategory === 'email' ? 'bg-[#ff6a00] text-black hover:bg-[#ff6a00]' : 'text-white/40 hover:text-white hover:bg-white/5'}`}>
            <Mail className="h-3.5 w-3.5"/> E-mail
          </Button>
          <Button variant="ghost" onClick={() => { setActiveCategory('webhooks'); setSelectedItem(null); }} className={`flex items-center gap-2 px-4 py-2 text-[10px] font-bold uppercase tracking-widest transition h-10 ${activeCategory === 'webhooks' ? 'bg-[#ff6a00] text-black hover:bg-[#ff6a00]' : 'text-white/40 hover:text-white hover:bg-white/5'}`}>
            <Zap className="h-3.5 w-3.5"/> Webhooks
          </Button>
          <Button variant="ghost" onClick={() => { setActiveCategory('offers'); setSelectedItem(null); }} className={`flex items-center gap-2 px-4 py-2 text-[10px] font-bold uppercase tracking-widest transition h-10 ${activeCategory === 'offers' ? 'bg-[#ff6a00] text-black hover:bg-[#ff6a00]' : 'text-white/40 hover:text-white hover:bg-white/5'}`}>
      ...
      <div className="grid gap-8 grid-cols-1 lg:grid-cols-12">
        ...
        <div className="lg:col-span-4 space-y-4">
          {activeCategory === 'email' ? (<Card className="bg-[#111] border-white/5">
              <CardHeader>
              ...
              <CardContent>
            ...
            </Card>) : activeCategory === 'webhooks' ? (<Card className="bg-[#111] border-white/5">
               ...
               <CardContent className="space-y-4">
                 {WEBHOOKS.map((webhook, idx) => (<div key={idx} className="p-3 bg-black/40 border border-white/5 rounded-lg group">
              ...
              {isLoading ? (Array(4).fill(0).map((_, i) => <div key={i} className="h-24 bg-white/5 animate-pulse rounded-lg border border-white/5"/>)) : (filtered.map((item) => {
                const status = validateStatus(item);
                return (<button key={item.id} onClick={() => { setSelectedItem(item); setOriginalItem(JSON.parse(JSON.stringify(item))); setTestResult(null); }} className={`w-full text-left p-4 rounded-xl border transition-all duration-300 group ${selectedItem?.id === item.id ? 'bg-[#ff6a00]/10 border-[#ff6a00] shadow-[0_0_20px_rgba(255,106,0,0.1)]' : 'bg-[#111] border-white/5 hover:border-white/20'}`}>
                      <div className="flex items-center justify-between mb-3">
        ...
        <div className="lg:col-span-8">
          {activeCategory === 'email' ? <EmailIntegrationPanel integrations={integrations}/> : activeCategory === 'offers' ? <OffersIntegrationPanel /> : (selectedItem && selectedItem.category !== 'resend') ? (<div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
              <Tabs defaultValue="config" className="w-full">
                  ...
                  <Card className="bg-[#111] border-white/5 overflow-hidden">
                    <CardHeader className="border-b border-white/5 bg-white/[0.02]">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                           ...
                           <Button variant="outline" size="sm" onClick={() => setSelectedItem({ ...selectedItem, status: !selectedItem.status })} className={`h-7 rounded-full px-4 border-none transition-all ${selectedItem.status ? 'bg-[#ff6a00] text-black shadow-[0_0_10px_rgba(255,106,0,0.3)]' : 'bg-white/10 text-white/40'}`}>
                    ...
                    <CardContent className="p-6 space-y-6">
                      <div className="grid gap-6 md:grid-cols-2">
                        {Object.keys(selectedItem.credentials).map((key) => (<div key={key} className="space-y-2">
                            ...
                            <Input type="password" value={selectedItem.credentials[key]} onChange={(e) => setSelectedItem({
                    ...selectedItem,
                    credentials: { ...selectedItem.credentials, [key]: e.target.value }
                })} className="bg-black/40 border-white/10 focus:border-[#ff6a00] h-11 text-sm font-mono text-[16px] md:text-sm" placeholder="sk-..."/>
                          </div>))}
                      </div>
                      ...
                      <div className="pt-6 border-t border-white/5">
                        ...
                        <div className="grid gap-6 md:grid-cols-2">
                          {Object.keys(selectedItem.settings).map((key) => (<div key={key} className="space-y-2">
                              <Label className="text-[10px] font-bold uppercase tracking-widest text-white/40">{key.replace(/([A-Z])/g, ' $1').trim()}</Label>
                              {key === 'environment' || key === 'testMode' ? (<select value={(String(selectedItem.settings[key]) === 'true') ? 'sandbox' : 'production'} onChange={(e) => {
                        ...
                        if (key === 'testMode') {
                            ...
                            setSelectedItem({
                                ...
                                </select>) : (<Input value={selectedItem.settings[key]} onChange={(e) => setSelectedItem({
                        ...selectedItem,
                  ...
                  <Card className="bg-[#111] border-white/5 overflow-hidden">
                    ...
                    <CardContent className="space-y-4">
                      <div className="flex flex-wrap gap-4 items-center justify-between p-4 bg-black/40 border border-white/5 rounded-xl">
                        ...
                        <Button onClick={handleTest} disabled={isTesting} className="bg-white/5 border border-white/10 hover:bg-white/10 text-[10px] font-bold uppercase tracking-widest h-10 px-6">
                          {isTesting ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-2"/> : <Zap className="h-3.5 w-3.5 mr-2 text-[#ff6a00]"/>}
                          Testar Conexão
                        </Button>
                      </div>

                      {testResult && (<div className={`p-5 rounded-xl border ${testResult.success ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'} animate-in slide-in-from-top-2 duration-300`}>
                  ...
                  <Card className="bg-[#111] border-white/5">
                    ...
                    <CardContent className="p-6">
                      <div className="space-y-4">
                        {(GUIDES[selectedItem.category] || ["Documentação em breve para este provedor."]).map((step, idx) => (<div key={idx} className="flex gap-4 p-4 bg-white/[0.02] border border-white/5 rounded-xl hover:border-white/10 transition group">
                            ...
                            <div className="space-y-1">
                              ...
                              {step.toLowerCase().includes('webhook') && (<Button variant="link" onClick={() => setActiveCategory('webhooks')} className="p-0 h-auto text-[10px] text-[#ff6a00] uppercase tracking-widest font-bold hover:no-underline hover:brightness-125">
                ...
                <TabsContent value="history" className="m-0 space-y-6">
                  <Card className="bg-[#111] border-white/5">
                    ...
                    <CardContent className="p-6">
                      <div className="space-y-4">
                        {historyLogs && historyLogs.length > 0 ? historyLogs.map((log) => (<div key={log.id} className="p-4 border border-white/5 bg-black/40 rounded-xl hover:border-white/10 transition">
                            <div className="flex items-center justify-between mb-2">
                          ...
                          </div>)) : (<div className="py-20 flex flex-col items-center justify-center text-white/10 border border-dashed border-white/10 rounded-xl">
            ...
            </div>) : (<div className="h-[600px] border border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center bg-white/[0.01] p-12 text-center animate-pulse">
...
function EmailIntegrationPanel({ integrations }) {
    const queryClient = useQueryClient();
    ...
    if (loadingSettings)
        return <div className="flex h-64 items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-[#ff6a00]"/></div>;
    return (<div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
        ...
        <TabsContent value="config" className="space-y-6 m-0">
          <Card className="bg-[#111] border-white/5">
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input defaultValue={settings?.from_name} id="from_name" className="bg-black/40 border-white/10 focus:border-[#ff6a00] h-11 text-sm"/>
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase tracking-widest text-white/40">E-mail do Remetente</Label>
                  <Input defaultValue={settings?.from_email} id="from_email" className="bg-black/40 border-white/10 focus:border-[#ff6a00] h-11 text-sm"/>
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase tracking-widest text-white/40">E-mail de Resposta (Reply-To)</Label>
                  <Input defaultValue={settings?.reply_to || ""} id="reply_to" className="bg-black/40 border-white/10 focus:border-[#ff6a00] h-11 text-sm"/>
                </div>
              </div>
            ...
            updateSettingsMutation.mutate({
                ...
                <Button variant="outline" onClick={() => settings && updateSettingsMutation.mutate({
            data: {
            ...
            <CardContent className="space-y-6">
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input placeholder="exemplo@email.com" value={testTo} onChange={(e) => setTestTo(e.target.value)} className="bg-black/40 border-white/10 focus:border-[#ff6a00] h-11"/>
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-bold uppercase tracking-widest text-white/40">Template para Testar</Label>
                  <select value={testTemplate} onChange={(e) => setTestTemplate(e.target.value)} className="w-full h-11 bg-black border border-white/10 rounded-lg text-white text-sm px-4 outline-none focus:border-[#ff6a00]">
              ...
              <Button disabled={!testTo || isSendingTest} onClick={() => {
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {loadingLogs ? (Array(5).fill(0).map((_, i) => <tr key={i} className="h-16 animate-pulse bg-white/[0.01]"/>)) : logs?.length === 0 ? (<tr>
                        <td colSpan={4} className="px-6 py-12 text-center text-xs text-white/20 uppercase font-bold tracking-widest">Nenhum log encontrado.</td>
                      </tr>) : (logs?.map((log) => (<tr key={log.id} className="hover:bg-white/[0.02] transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <Badge variant="outline" className={`text-[8px] uppercase tracking-widest border-none ${log.status === 'sent' ? 'text-emerald-400 bg-emerald-400/10' :
...
function ResendConfigTab({ integration: initialIntegration }) {
    const queryClient = useQueryClient();
    ...
    useEffect(() => {
        if (initialIntegration) {
            ...
            setIntegration({
                id: '',
                name: 'Resend',
                type: 'ia', // category logic in functions handles it
                category: 'resend',
                status: false,
                credentials: { apiKey: '' },
                settings: {}
    ...
    const handleSave = async () => {
        ...
        try {
            await saveIntegrationFn({
                data: {
                    ...integration,
                    type: 'ia' // Backend expects this for integrations table
                }
            });
    ...
    const handleTest = async () => {
        ...
        try {
            ...
            if (result.success)
                ...
                toast.error("Falha na validação: " + result.message);
        }
        catch (err) {
            toast.error(err.message || "Erro no teste");
        }
    ...
    if (!integration)
        return null;
    return (<Card className="bg-[#111] border-white/5">
      <CardHeader className="border-b border-white/5 bg-white/[0.02]">
        <div className="flex items-center justify-between">
          <div>
          ...
          <Button variant="outline" size="sm" onClick={() => setIntegration({ ...integration, status: !integration.status })} className={`h-7 rounded-full px-4 border-none transition-all ${integration.status ? 'bg-[#ff6a00] text-black' : 'bg-white/10 text-white/40'}`}>
      ...
      <CardContent className="p-6 space-y-6">
        <div className="space-y-2">
          ...
          <Input type="password" value={integration.credentials.apiKey || ''} onChange={(e) => setIntegration({
            ...integration,
            credentials: { ...integration.credentials, apiKey: e.target.value }
        })} placeholder="re_..." className="bg-black/40 border-white/10 focus:border-[#ff6a00] h-11 text-sm font-mono"/>
        </div>
        ...
        <div className="pt-4 flex flex-col gap-4">
          ...
          {testResult && (<div className={`p-4 rounded-xl border ${testResult.success ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'}`}>
...
function OffersIntegrationPanel() {
    ...
    const saveOfferSettings = async (updates) => {
        try {
            const existingSettings = offerSettings?.settings || {};
            const newSettings = {
                ...existingSettings,
            ...
            if (error)
                throw error;
            if (updates.status !== undefined && typeof window !== 'undefined' && window.togglePostPurchaseOfferPopup) {
                window.togglePostPurchaseOfferPopup(updates.status);
            }
        ...
        <CardHeader className="border-b border-white/5 bg-white/[0.02]">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 bg-black/40 p-1.5 rounded-full border border-white/5">
              ...
              <Button variant="ghost" size="sm" onClick={() => saveOfferSettings({ status: !isEnabled })} className={`h-7 w-12 rounded-full p-0 transition-all relative ${isEnabled ? 'bg-emerald-500/20' : 'bg-white/5'}`}>
          ...
          <div className="grid gap-8 md:grid-cols-2">
            <div className="space-y-4">
              ...
              <div className="relative group">
                <Input type="number" min={15} max={100} defaultValue={currentDiscount} id="discount_input" className="bg-black/60 border-white/10 focus:border-[#ff6a00] h-14 text-xl font-bold pl-12 rounded-2xl transition-all"/>
